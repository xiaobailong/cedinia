...
- 代号：`I`=`issues-solved.md`　`P`=`pitfalls.md`　`D`=`decisions.md`
- `archive/` **默认不读**（有指针才读）；**写**条目看 `WRITING.md`；规矩见 `.clinerules/memory-bank.md`
- 上限：本索引 3KB，`I`/`P`/`D` 各 20KB（见 `ADR-012`；超了先归档）
- **迁移自 ClipboardMerger**（2026-09-23，同机同工具链）：编号沿用原号，条目内标「来源」；新条目从最大号 +1 开始

## 症状 → 条目

| 症状 / 报错 | 条目 | 文件 |
| --- | --- | --- |
| 构建日志空 / 缺内容 / 路径为空 | `ISSUE-001` | I |
| 构建失败也消耗一个 patch 版本号（跳号） | `ISSUE-002` | I |
| 改 `build.bat` / `gh-release.bat` 的发布 / 日志 / 版本流程 | `ADR-009` / `ADR-010` / `ADR-011` / `ADR-013`、`ISSUE-004`（未迁移见 `ADR-012`） | D |
| 校验不 push / 推送瞬断失败（含 `Patch …: EOF`）/ `gh` / Release 出问题 / 同名资产 422 | `ADR-009` / `ADR-011`、`ISSUE-005`、`PIT-027` | D + I + P |
| 只补发 / 重发 GitHub Release、`gh-release.bat` 用法 | `ADR-011` | D |
| 临时文件放哪、怎么清；仓库根堆临时产物 | `ADR-002` | D |
| 知识库按需读、`.clineignore` / 会话压缩 | `ADR-001` / `ADR-007`（归档） | D |
| 知识库 / 规则 / 发布脚本从哪来、编号不重排 | `ADR-012` | D |
...
| 终端抓不到输出 / 命令互相打断（长任务被 `Ctrl+C` 回收）/ 轮询被掐断 | `PIT-008`、`PIT-009`、`PIT-018`、`PIT-029` | P |
